...
```{r}
temp_day_extended <- temp %>%
  # keep only a single observation for every 30-min interval
  # use the most extreme temp if above/below the low/high bounds
  # otherwise just use the last value
  dplyr::arrange(DATETIME) %>%
  dplyr::mutate(lower_temp = min(TEMPERATURE),
                higher_temp = max(TEMPERATURE),
                latest_temp = last(TEMPERATURE),
                .by = rounded_datetime) %>%
  dplyr::mutate(TEMPERATURE = case_when(lower_temp<TEMP_LOW ~ lower_temp,
                                        higher_temp>TEMP_HIGH ~ higher_temp,
                                        TRUE ~ latest_temp)) %>%
...
combined_dataset <- forecastdemand %>%
  ...
  dplyr::inner_join(
    temp %>% 
      # ignore observations which are taken outside of every half hour
  ...
  dplyr::mutate(
    ...
    temp_control = case_when(
      TEMPERATURE < TEMP_LOW ~ "Low",
      TEMPERATURE > TEMP_HIGH ~ "High",
      TRUE ~ "Mid"),
    temp_control = factor(temp_control, c("Low", "Mid", "High")),
...
combined_dataset_day <- combined_dataset %>%
  dplyr::mutate(date = date(rounded_datetime),
                month = month(rounded_datetime)) %>%
  dplyr::summarise(
    # number of observations made for the day
    n_obv = n(),
    # number of intervals with high or low temp recorded
    n_high = sum(temp_control=="High"),
    n_low = sum(temp_control=="Low"),
    # find the min and max temperatures during the day
    min_temp = min(TEMPERATURE),
    max_temp = max(TEMPERATURE),
    # find the sum of total and forecast demand during the day
    ...
    mae_high = mean(abs(error * (temp_control=="High"))),
    mae_mid = mean(abs(error * (temp_control=="Mid"))),
    mae_low = mean(abs(error * (temp_control=="Low"))),
    # RMSE for times under different temp conditions during the day
    rmse_high = sqrt(mean(error^2 * (temp_control=="High"))),
    rmse_mid = sqrt(mean(error^2 * (temp_control=="Mid"))),
    rmse_low = sqrt(mean(error^2 * (temp_control=="Low"))),
    .by = c(year, month, date)
  ) %>%
  dplyr::mutate(
    # set MAE, RMSE by temp conditions to NA if there is no low/high temp
    mae_high = if_else(n_high>0, mae_high, NA),
    mae_low = if_else(n_low>0, mae_low, NA),
    rmse_high = if_else(n_high>0, rmse_high, NA),
    rmse_low = if_else(n_low>0, rmse_low, NA)
  )
```
...
ggplot(combined_dataset_day %>%
         mutate(actual_pr = percent_rank(sum_actual)), 
...
```{r}
ggplot(combined_dataset_day %>% dplyr::filter(n_high>0), 
       aes(mae_mid, mae_high, col=max_temp)) +
  geom_point() +
...
```{r}
ggplot(combined_dataset_day %>% dplyr::filter(n_low>0), 
       aes(mae_mid, mae_low, col=min_temp)) +
  geom_point() +
  scale_color_gradient(
